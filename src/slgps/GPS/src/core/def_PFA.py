...
import numpy as np
from slgps.GPS.src.ct.def_ct_tools import * 
from slgps.GPS.src.ct.senkin import senkin
import time
...
def PFA_algo(soln, RR, crit, seeds, path_save=None, overwrite=False):
	...
	if if_compute:
		...
		for i_rxn in range(n_rxn):
			...
			for s in list(rxn(i_rxn).reactants.keys()):
				if s not in list(s_mu.keys()):
					s_mu[s] = 0
				s_mu[s] -= rxn(i_rxn).reactants[s]
			
			for s in list(rxn(i_rxn).products.keys()):
				if s not in list(s_mu.keys()):
					s_mu[s] = 0
			...
			for A in list(s_mu.keys()):
				id_A = soln.species_index(A)
				for B in list(s_mu.keys()):
