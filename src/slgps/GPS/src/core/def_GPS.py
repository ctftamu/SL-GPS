...
from slgps.GPS.src.core.def_yen import k_shortest_paths as yen
from .def_tools import *
import os
from slgps.GPS.src.core.def_build_graph import *
...
def find_hubs(score, iso, alpha, gamma=None):
    ...
    if iso == None:
        iso_kept = None
        n_hub = sum([score[node] > alpha for node in list(score.keys())])
        ...
        for iso_name in list(iso.keys()):
            ...
            for sp in members:
                if sp in list(score.keys()):# and score[sp] > iso_alpha:
...
def GPS_algo(soln, flux_graph, source, target, path_save=None, \
    ...
    score = {node: max(in_deg[node], out_deg[node]) / norm_deg for node in list(dict(in_deg).keys())}
    ...
    if target is not None:
        ...
        for hub in hubs:
            if hub not in list(hub_GP.keys()):
                hub_GP[hub] = {}
                ...
                for p0 in pp0:
                    ...
                    for p1 in pp1:
                        ...
                        if no_cycle:
                            ...
                            if GP not in list(global_path.keys()):
                                global_path[GP] = {}
                                global_path[GP]['member'] = p

        # ---------------------------------     
        st_list = []
        for GP in list(global_path.keys()):
            for sp in global_path[GP]['member']:
                if sp not in list(species_kept.keys()):
                    species_kept[sp] = {}
    ...
    GPS_results['summary']['n_species_kept'] = len(list(species_kept.keys()))
    GPS_results['summary']['n_global_path'] = len(list(global_path.keys()))
