...
def make_data(fuel, mech_file, end_threshold, ign_HRR_threshold_div, ign_GPS_resolution, 
    ...
    # Begin collecting GPS reduced mechanisms
         ...
         bin_array = None  # Initialize bin_array variable outside the loop
         data_array = None  # Initialize data_array variable outside the loop

         while t + Dt < auto_ign_det[0]['axis0'][-1]:
           count += 1

        # Decide which GPS interval to use based on the ignition interval
        ...
        # Initialize or grow arrays of species and state vector data
           if count == 1:
            ...
            data_array = np.array([[auto_ign_det[0]['temperature'][findTimeIndex(auto_ign_det[0]['axis0'], t)], pres, auto_ign_det[0]['mole_fraction'][findTimeIndex(auto_ign_det[0]['axis0'], t)].tolist()]], dtype=object)
           else:
            bin_array = np.append(bin_array, bin_spec_list, axis=0)
            new_data = [[auto_ign_det[0]['temperature'][findTimeIndex(auto_ign_det[0]['axis0'], t)], pres, auto_ign_det[0]['mole_fraction'][findTimeIndex(auto_ign_det[0]['axis0'], t)].tolist()]]
