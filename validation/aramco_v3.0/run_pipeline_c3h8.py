"""
SL-GPS Validation — AramcoMech 3.0 with Propane (C3H8)
=======================================================
Runs the full pipeline for propane fuel:
...
DATA_DIR = os.path.join(BASE_DIR, 'data', 'train_c3h8')
MODELS_DIR = os.path.join(BASE_DIR, 'models', 'c3h8')
RESULTS_DIR = os.path.join(BASE_DIR, 'results', 'c3h8')
PLOTS_DIR = os.path.join(BASE_DIR, 'results', 'c3h8')
...
# Propane/air composition ranges
SPECIES_RANGES = {
    'C3H8': (0.01, 0.06),
    'O2':   (0.10, 0.25),
    'N2':   (0.60, 0.80),
...
# Input species for propane combustion (includes C3 intermediates)
INPUT_SPECS = [
    'C3H8', 'O2', 'H2O', 'CO2', 'CO', 'OH', 'H', 'H2',
    'CH4', 'C2H4', 'C2H6', 'CH2O', 'C3H6', 'HO2'
]

# Validation cases for propane (label, T0, P_atm, phi, t_end)
VALIDATION_CASES = [
    # --- Baseline cases ---
    ('C3H8_T1000_P1',   1000,  1.0, 1.0, 0.2),
    ('C3H8_T1200_P10',  1200, 10.0, 1.0, 0.02),
    ('C3H8_T1500_P1',   1500,  1.0, 1.0, 0.005),
    ('C3H8_T1500_P10',  1500, 10.0, 1.0, 0.002),
    # --- More difficult cases ---
    ('C3H8_T900_P20',    900, 20.0, 1.0, 0.5),     # Low-T, high-P (NTC regime)
    ('C3H8_T1200_P1_rich', 1200, 1.0, 1.3, 0.05),  # Rich mixture
    ('C3H8_T1300_P5_lean', 1300, 5.0, 0.7, 0.01),  # Lean mixture
    ('C3H8_T1800_P20',  1800, 20.0, 1.0, 0.001),   # High-T, high-P (fast ignition)
]
...
def step1_generate_data():
    print("\n" + "=" * 70)
    print("  STEP 1: Training Data Generation (C3H8 / GPS)")
    print("=" * 70)
...
def step2_train():
    print("\n" + "=" * 70)
    print("  STEP 2: Neural Network Training (C3H8)")
    print("=" * 70)
...
def step3_validate():
    print("\n" + "=" * 70)
    print("  STEP 3: Validation Simulations (C3H8)")
    print("=" * 70)
    ...
    for i, (label, T0, P_atm, phi, t_end) in enumerate(VALIDATION_CASES):
        ...
        case_results = {'config': {'label': label, 'T0': T0, 'P_atm': P_atm, 'phi': phi, 't_end': t_end, 'fuel': 'C3H8'}}
    ...
    print(f"\n  ✓ All C3H8 validation cases complete")
    return True
...
def step4_plots():
    print("\n" + "=" * 70)
    print("  STEP 4: Plot Generation (C3H8)")
    print("=" * 70)
    ...
    # Load only C3H8 results
    c3h8_cases = []
    for label, *_ in VALIDATION_CASES:
        fpath = os.path.join(RESULTS_DIR, f'{label}.pkl')
        if os.path.isfile(fpath):
            with open(fpath, 'rb') as f:
                c3h8_cases.append(pickle.load(f))

    if not c3h8_cases:
        print("  No C3H8 results to plot!")
        return False

    # Individual case plots
    for case_data in c3h8_cases:
        ...
        fig.suptitle(
            f"AramcoMech 3.0 — Propane (C₃H₈) — {label}\n"
        ...
        # Species
        plot_specs = ['C3H8', 'O2', 'CO2', 'H2O', 'CO', 'C2H4', 'CH4']
    ...
    fig.suptitle('AramcoMech 3.0 — Propane (C₃H₈) Validation Summary', fontsize=12, fontweight='bold')

    labels_list = [c['config']['label'].replace('C3H8_', '') for c in c3h8_cases]
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(PLOTS_DIR, 'C3H8_summary.png'), dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  Saved: C3H8_summary.png")

    print(f"\n  ✓ All C3H8 plots saved")
    return True
...
if __name__ == '__main__':
    print("╔══════════════════════════════════════════════════════════════════════╗")
    print("║  SL-GPS VALIDATION — AramcoMech 3.0 / Propane (C3H8)              ║")
    ...
    steps = [
        ("Data Generation (C3H8)", step1_generate_data),
        ("NN Training (C3H8)", step2_train),
        ("Validation (C3H8)", step3_validate),
        ("Plotting (C3H8)", step4_plots),
    ]
    ...
    print(f"\n{'═' * 70}")
    print(f"  ✓ C3H8 PIPELINE COMPLETE — {total:.1f}s ({total/60:.1f} min)")
    print(f"{'═' * 70}")
